...
def test_get_model_by_id_success():
    """Test get_model_by_id returns correct model information."""
    from backend.services import model_management_service as svc
    ...
    with mock.patch.object(svc, "get_model_records", return_value=records), \
        ...
        result = svc.get_model_by_id(1, "t1")

        assert result is not None
        assert result["model_id"] == 1
        assert result["model_name"] == "llama-2"
        assert result["model_repo"] == "huggingface"
        assert result["display_name"] == "LLaMA 2"
        assert result["api_key"] == "test_key"
        assert result["base_url"] == "https://api.huggingface.co"
        assert result["connect_status"] == "operational"
...
def test_get_model_by_id_not_found():
    ...
    with mock.patch.object(svc, "get_model_records", return_value=[]):
        result = svc.get_model_by_id(999, "t1")
        assert result is None


def test_get_model_by_id_exception():
    """Test get_model_by_id handles exceptions properly."""
    from backend.services import model_management_service as svc
