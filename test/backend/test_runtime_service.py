...
with patch('elasticsearch.Elasticsearch', return_value=MagicMock()), \
    ...
    # Mock dotenv before importing runtime_service
    with patch('dotenv.load_dotenv'):
        # Mock logging configuration
        with patch('utils.logging_utils.configure_logging'), \
                patch('utils.logging_utils.configure_elasticsearch_logging'):
            from runtime_service import startup_initialization


class TestMainService:
    """Test cases for runtime_service module"""

    @pytest.mark.asyncio
    @patch('runtime_service.initialize_tools_on_startup', new_callable=AsyncMock)
    @patch('runtime_service.logger')
...
class TestMainServiceModuleIntegration:
    ...
    @patch('runtime_service.APP_VERSION', 'test_version_1.2.3')
    @patch('runtime_service.initialize_tools_on_startup', new_callable=AsyncMock)
    @patch('runtime_service.logger')
